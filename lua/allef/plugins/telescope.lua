return {
    ...
    config = function()
        ...
        vim.keymap.set(
            "n",
            "<C-p>",
            '<Cmd>lua require("telescope.builtin").find_files({ no_ignore = true, no_ignore_parent = true, hidden = true, previewer = false, find_command = { "rg", "--files", "--hidden", "--glob", "!**/.git/*", "--glob", "!**/node_modules/*" } })<CR>',
            options
        )
        vim.keymap.set(
            "n",
        ...
        vim.keymap.set(
            "n",
            "<leader>ff",
            '<Cmd>lua require("telescope").extensions.file_browser.file_browser({ path = "%:p:h", cwd = Telescope_buffer_dir(), respoect_git_ignore = false, hidden = true, grouped = true, initial_mode = "normal" })<CR>',
            options
        )
    end,
}
